...
/** MOD */
Thread ros_thread;
volatile bool pump_command = false;
// Vars for activating the pump + LED feedback
bool pump_active;

/*
 * Allows ROS to publish a command to turn the pump on/off
 * 
 * @param on    Should the pump be spraying
 */
void subscriberCallback(const std_msgs::Bool &on)
{
    pump_active = on.data;
    pump_command = true;
}
...
int main()
{
    ...
    ros::Publisher pub("pump_status", &str_output);
    // We instantiate publisher object with "pump_command" topic and attach
    // callback for subscriber event (when user sends something).
    ros::Subscriber<std_msgs::Bool> sub("pump_command", subscriberCallback);
    // subscribe to topic + advertise publisher
    nh.advertise(pub);
    nh.subscribe(sub);
